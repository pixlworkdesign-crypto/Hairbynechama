...
Use the Admin button in the footer to update styles/prices, upload gallery photos, set regular times for each weekday, and manage the bookings calendar. The default admin password is `nechama123`. New booking requests are saved as pending in the Admin calendar. When a booking is submitted on Vercel, the site sends an email to `nechamabrenig@gmail.com` with all booking details and an approval link back to the website. That approval link carries the booking details so the admin browser can load it into the calendar as pending. When you approve a booking, it is marked confirmed and the site sends a confirmation email to the customer.
